...
async function seedDatabase() {
  const dbPath = path.join(__dirname, '../database/weather.db');
  ...
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run('CREATE INDEX IF NOT EXISTS idx_regions_name_ko ON regions(name_ko)', (err) => {
        if (err) console.error('Error creating name index:', err);
      });

      db.run('CREATE INDEX IF NOT EXISTS idx_regions_province ON regions(province)', (err) => {
        if (err) console.error('Error creating province index:', err);
      });

      db.run('CREATE INDEX IF NOT EXISTS idx_regions_coordinates ON regions(latitude, longitude)', (err) => {
      ...
      db.run('DELETE FROM regions', (err) => {
        ...
        for (const region of koreanRegions) {
          stmt.run([
            ...
            if (insertedCount === koreanRegions.length) {
              stmt.finalize((err) => {
                if (err) {
                ...
                db.get('SELECT COUNT(*) as count FROM regions', (err, row) => {
                  ...
                  console.log(`Total regions in database: ${row.count}`);
